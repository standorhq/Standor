  ...
  useEffect(() => {
    ...
    const onDocumentKey = (e) => {
      if (e.code === "KeyF") {
        if (screen) {
          setFullScreen((prev) => {
            if (!prev) pointerLockControls.current.unlock();
            return !prev;
          });
        }
      }
    ...
    <>
      <div className={`absolute w-screen h-screen z-10 ${fullScreen ? "" : "hidden"}`}>
        <div className="w-full h-full">
          <video id="screenVideo" autoPlay playsInline className="w-full h-full object-fill" />
        </div>
      </div>
      <div className={`absolute right-0 top-1 z-10 ${screen && !fullScreen ? "" : "hidden"}`}>
        <div className="bg-[#5c89d1] rounded p-2">
          <p className="text-white text-xs">
            <span className="text-[#42f563] text-base">{name}</span> is sharing screen
          </p>
