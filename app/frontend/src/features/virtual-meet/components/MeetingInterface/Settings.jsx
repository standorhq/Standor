...
const Settings = ({ setBoxes, boxes }) => {
  ...
  const [deviceSections, setDeviceSections] = useState({ audio: false, video: false });
    ...
    const check = async () => {
      if (deviceSections.audio) {
        ...
        if (audioPermission.state === 'prompt') {
          ...
          alert("Please allow microphone access from browser to see all audio devices.");
          setDeviceSections({ ...deviceSections, audio: false });
          return;
        }
        setLoading({ audio: true, video: false });
        navigator.mediaDevices.enumerateDevices().then((devices) => {
          setAudioDevices(devices.filter((d) => d.kind === "audioinput" && !d.label.includes("Communications")));
        ...
        if (cameraPermission.state === 'prompt') {
          ...
          alert("Please allow camera access from browser to see all video devices.");
        }
  ...
  return (
    <div className={`fixed bottom-[70px] right-2 z-20 bg-[#5c89d1] rounded w-72 h-[55%] flex flex-col ${boxes.settings ? "" : "hidden"}`}>
      <div className="flex justify-center border-b-2 rounded"><div>Settings</div></div>
      ...
      <div className="mt-1 p-1 flex-col text-center flex-1 overflow-y-scroll no-scrollbar">
        <button onClick={() => setDeviceDropDown(!deviceDropDown)} className="flex items-center justify-between p-2 rounded-lg w-full bg-[#535a6d] hover:bg-[#6f778f]">
        ...
        <div className={`pl-2 ${deviceDropDown ? "" : "hidden"}`}>
          ...
          <div className={`${deviceSections.audio && !loading.audio ? "" : "hidden"} w-full p-1`}>
            {audioDevices.map((value, index) => (
              <button key={index} onClick={() => setDevice((prev) => ({ ...prev, audio: value.deviceId }))}
                className={`flex items-center justify-center bg-[#535a6d] py-1 w-full border hover:bg-[#6f778f] text-xs ${index === audioDevices.length - 1 ? "rounded-b-lg" : ""} ${index === 0 ? "rounded-t-lg" : ""}`}>
                <div>{value.label}</div>
                {value.deviceId == device.audio ? <div className="text-xs text-gray-400 mx-1"><FaCheck color="#39fa73" /></div> : ""}
              </button>
            ))}
          </div>
          ...
          <div className={`${deviceSections.video && !loading.video ? "" : "hidden"} w-full p-1`}>
            {videoDevices.map((value, index) => (
              <button key={index} onClick={() => setDevice((prev) => ({ ...prev, video: value.deviceId }))}
                className={`flex items-center justify-center bg-[#535a6d] py-1 w-full border hover:bg-[#6f778f] text-xs ${index === videoDevices.length - 1 ? "rounded-b-lg" : ""} ${index === 0 ? "rounded-t-lg" : ""}`}>
                <div>{value.label}</div>
                {value.deviceId == device.video ? <div className="text-xs text-gray-400 mx-1"><FaCheck color="#39fa73" /></div> : ""}
              </button>
            ))}
          </div>
        </div>
