...
const ChatBox = ({ setBoxes, boxes, setChatDot }) => {
  ...
  const [notifications, setNotifications] = useState({ show: false, message: "", name: "", color: "" });
  ...
  useEffect(() => {
    const getData = () => {
      ...
      if (data) {
        ...
        setChats(data);
        if (!boxes.chat) {
          const truncatedMessage = data[0].message.length > 30
          ...
          setChatDot(true);
          setTimeout(() => setNotifications({ show: false, message: "" }), 4000);
        }
      }
    };
    document.addEventListener("chat", getData);
    return () => document.removeEventListener("chat", getData);
  }, [boxes.chat]);

  useEffect(() => {
    if (boxes.chat) setNotifications({ show: false, message: "" });
  }, [boxes.chat]);
  ...
  const onMessage = () => {
    ...
    if (!data) {
      ...
      if (data[0].id === socket.id) curr = { ...curr, prev: true };
      data.unshift(curr);
    }
  ...
  return (
    <>
      <div className={`fixed bottom-[70px] right-2 z-20 bg-[#5c89d1] rounded w-72 h-[55%] ${boxes.chat ? "" : "hidden"}`}>
        <div className="flex flex-col h-full">
          <div className="flex justify-between px-2 mt-1 border-b-2 rounded pb-1">
            <p>All Chat</p>
            <button className="rounded-full hover:bg-gray-400" onClick={() => setBoxes({ ...boxes, chat: false })}>
              <IoClose size={23} />
            </button>
          </div>
          <div className="w-full flex-1 mt-1 px-2 overflow-y-scroll no-scrollbar flex flex-col-reverse">
            {chats.map((chat, index) => (
              <div key={index}>
                <p className={`p-1 ${chat.prev ? "mt-[0.1rem]" : "mt-1"} max-w-[80%] rounded-lg break-words ${
                  chat.id === socket.id ? "float-end bg-[#498d73]" : "float-start bg-[#445b80]"
                }`}>
                  <span className="flex flex-col">
                    {chat.id !== socket.id && !chat.prev && (
      ...
      {notifications.show && (
        <div className="fixed bottom-[70px] right-12 z-20 bg-[#5c89d1] rounded w-52 max-h-32 flex flex-col break-words text-center">
          ...
          <div className="bg-[#445b80] my-2 mx-1 p-1 flex flex-col text-left rounded-lg">
            <span className="text-[10px]" style={{ color: notifications.color }}>{notifications.name}</span>
