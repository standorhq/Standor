...
const Info = () => {
  ...
  const baseUrl = `${window.location.origin}/virtual-meet/`;
  ...
  const copyLink = async () => {
    ...
    try {
      ...
      setCheckLink(true);
      timeoutLink = setTimeout(() => setCheckLink(false), 2000);
  ...
  const copyId = async () => {
    ...
    try {
      ...
      setCheckId(true);
      timeoutId = setTimeout(() => setCheckId(false), 2000);
    } catch { alert("Failed to copy link"); }
  };
  ...
  return (
    <>
      {!showModal ? (
        <button className="fixed top-2 left-1 z-10 w-[3rem] flex text-center justify-center" onClick={() => setShowModal(true)}>
        ...
        <div className="fixed top-1 left-1 z-10 2xl:max-w-[20%] xl:max-w-[25%] lg:max-w-[30%] md:max-w-[35%] max-w-[55%] p-2 bg-[#5c89d1] rounded flex-col text-center">
          ...
          <div className="grid grid-rows-4 grid-cols-2 mt-4 gap-2">
            ...
            <div className="flex">
              ...
              <div className={`ml-1 mt-2 ${checkId ? "" : "hidden"}`}><FaCheck color="#39fa73" /></div>
            </div>
            ...
            <div className="row-span-3 break-words">
              ...
              <div className="flex">
                <button className="text-sm flex text-center mt-2 max-w-[fit-content] bg-[#dfdcdc] rounded px-1 active:opacity-50" onClick={copyLink}>
                  ...
                  <span className="mt-[0.2rem]"><MdOutlineContentCopy color="#0000EE" /></span>
                </button>
                <div className={`ml-1 mt-2 ${checkLink ? "" : "hidden"}`}><FaCheck color="#39fa73" /></div>
              </div>
            </div>
          </div>
