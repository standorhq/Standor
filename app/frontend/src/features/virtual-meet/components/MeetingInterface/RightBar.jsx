...
const RightBar = () => {
  ...
  const [boxes, setBoxes] = useState({ chat: false, settings: false, leave: false });
  ...
  const disconnectAlone = () => {
    ...
    setPlayerKeys([]);
    setPeerConn([]);
    setTimeout(() => { setLoading(false); navigate("/dashboard", { replace: true }); }, 500);
  };
  ...
  const onLeave = () => setBoxes((prev) => ({ chat: false, settings: false, leave: !prev.leave }));
  const onChat = () => { setBoxes((prev) => ({ settings: false, leave: false, chat: !prev.chat })); setChatDot(false); };
  const onSettings = () => setBoxes((prev) => ({ chat: false, leave: false, settings: !prev.settings }));

  return (
    <>
      {loading && <div className="fixed flex items-center justify-center h-screen z-20"><LoaderSync /></div>}
      <div className="fixed bottom-3 right-2 z-20 flex">
        ...
        <button onClick={onSettings} className="bg-gray-300 px-2 h-12 rounded-[100px] flex justify-center text-center hover:bg-white ml-2">
          <div className="mt-[10px] ml-[0.1rem]"><IoSettings size={30} color="#5c89d1" title="Settings" /></div>
        </button>
        <button onClick={onLeave} className="bg-gray-300 px-2 h-12 rounded-[100px] flex justify-center text-center hover:bg-white ml-2">
          <div className="mt-2 ml-[0.1rem]"><ImPhoneHangUp size={30} color="#db3954" /></div>
        </button>
      </div>
      <div><ChatBox setBoxes={setBoxes} boxes={boxes} setChatDot={setChatDot} /></div>
      <div><Settings setBoxes={setBoxes} boxes={boxes} /></div>
      <div className={`fixed bottom-[70px] right-1 z-20 bg-[#5c89d1] rounded min-w-52 ${boxes.leave ? "" : "hidden"}`}>
        ...
        <div className="py-3 px-8 flex flex-col mt-2">
          ...
          <button className={`bg-[#db3954] mt-4 rounded px-2 ${isAdmin ? "hover:scale-110 duration-300" : "opacity-60"}`} disabled={!isAdmin} onClick={disconnectEveryone}>End Meeting for All</button>
        </div>
      </div>
    </>
  );
