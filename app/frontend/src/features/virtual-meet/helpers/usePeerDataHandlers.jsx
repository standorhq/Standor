...
const usePeerDataHandlers = (
    ...
    peer.current.on("call", (call) => {
      ...
      call.on("stream", (userStream) => {
        if (call.metadata.type === "screen") {
          ...
          if (!videos[call.peer]) {
            setVideos((prev) => ({ ...prev, [call.peer]: userStream }));
          }
        } else if (call.metadata.type === "audio") {
          if (!audios[call.peer]) {
            setAudios((prev) => ({ ...prev, [call.peer]: userStream }));
          }
        }
      });
    });
  ...
  const dataChannel = (conn, data) => {
    ...
    if (data.type === "audio") {
      setAudioIcon((prev) => ({ ...prev, [data.socketId]: data.audio }));
    ...
    setPlayerKeys((prev) => {
      ...
      if (!socketIds.includes(id)) {
        if (screenStreamRef.current) {
          conn.send({ type: "screen", screen: true, peerId: peer.current.id });
