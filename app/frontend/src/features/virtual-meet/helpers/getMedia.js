...
export const getMediaStreamAudio = async (
  audioStreamRef, playerKeys, peerConn, socket, peer, deviceId
) => {
  const promise = new Promise((resolve) => {
    navigator.mediaDevices
...
export const getMediaStreamScreen = (screenStreamRef, playerKeys, peerConn, peer) => {
  const promise = new Promise((resolve) => {
    navigator.mediaDevices
      .getDisplayMedia({
...
export const connectToNewUser = (id, stream, peer, type) => {
  ...
  peer.current.call(id, stream, options);
};
